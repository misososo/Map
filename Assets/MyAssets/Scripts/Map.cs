...
public class Map : MonoBehaviour
{
    ...
    void SettingRoomType()
    {
        ...
        for (int i = 0; i < gimmickRoomNum; ++i)
        {
            random = Random.Range(0, posX.Count);

            room = Instantiate(gimmickRoom, roomTileMap.GetCellCenterWorld(new Vector3Int(posX[random], posY[random], 0)), Quaternion.identity);
            room.SetRoomPos(posX[random], posY[random]);
            room.SetId(roomId);
            roomId++;
